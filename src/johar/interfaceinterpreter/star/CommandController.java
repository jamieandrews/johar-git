...
/**
 * The Command Controller class interacts with the Widgets; It performs any action 
 * required by the user, while handling all events generated by the Widgets; It 
 * interacts with the Gem via GemSetting and the Idf to serve requests raised from the 
 * Widgets.
 */
public class CommandController implements CommandControllerBase {
	...
	private void initializeStage(){
		commandDialog.initializeStageGUI(currentStage);
	}
	...
	private boolean stageWrapUp() {
		paramWidgetsMap = commandDialog.getStageGUI(currentStage).getParameterWidgets();
	...
	//Table row selection procedure for the Help Dialog box
	public void tableRowClicked(int helpState) {
		try {
			...
			if (helpState == 0){
				selectedRow = helpDialog.getTopLevelTableSelectedRow();
				if (selectedRow != -1){
					commandLabel = helpDialog.getTopLevelTable().getModel().getValueAt(selectedRow, 0).toString();
					selectedHelpCommand = idfAnalyzer.getIdfCommandByLabel(commandLabel);
					
					helpDialog.showCommandState(selectedHelpCommand);
				}
			}
			else if (helpState == 1){
				selectedRow = helpDialog.getParamsTableSelectedRow();
				if (selectedRow != -1){
					paramLabel = helpDialog.getParamsTable().getModel().getValueAt(selectedRow, 0).toString();
					selectedParam = idfAnalyzer.getIdfParameterByLabel(selectedHelpCommand, paramLabel);
					
					helpDialog.showParameterState(selectedHelpCommand, selectedParam, false);
				}
			}
			else if (helpState == 2){
				selectedRow = helpDialog.getQuestionsTableSelectedRow();
				
				if (selectedRow != -1){
					paramLabel = helpDialog.getQuestionsTable().getModel().getValueAt(selectedRow, 0).toString();
					selectedParam = idfAnalyzer.getIdfQuestionByLabel(selectedHelpCommand, paramLabel);
					
					helpDialog.showParameterState(selectedHelpCommand, selectedParam, true);
				}
			}
		} catch (Exception e) {
			MessageDialog.showError("An error occurred while performing the requested operation. [Error Details: "
	...
	public void commandMenuItemClickedStar(String commandName) {
		try {
			if (commandName.equals("help")){
				helpDialog = new HelpDialog(_idf, this);			
			}
		} catch (Exception e) {
			MessageDialog.showError("An error occurred while performing the requested operation. [Error Details: "
					+ e.getMessage() + "]");
	...
	//User clicks the Back button on the Help Dialog box
	public void helpBackButtonClicked() {
		try {
			if (helpCurrentState == 1){
				helpCurrentState = 0;
				helpDialog.showTopLevelState();
			}			
	...
	//User clicks the OK button on the Help Dialog box
