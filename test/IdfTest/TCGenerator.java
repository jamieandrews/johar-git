...
public class TCGenerator {
	...
	FILE_PATH = BASE_PATH + "Main IDF File" + File.separator;
   	FILE_PATH2 = BASE_PATH + "Valid IDF Files" + File.separator;
    	FILE_PATH3 = BASE_PATH + "Invalid IDF Files" + File.separator;
    ...
    public void validateIdf(String idfFile){
    	try{    	
    		...
    		if (status){
	            ...
	            writerError.append("\n");
	            writerError.append("*****BEGIN: Errors detected in IDF file\n");
	            writerError.append(errorMsg);
		    writerError.append("*****END: Errors detected in IDF file\n");	            
    		}            
    	}
    	catch(Exception ex){
    		System.out.println(ex.getMessage());
